...
- This machine, Windows: `C:\Users\1111\.openclaw\workspace\tuya-lights`
...
The old Python implementation is legacy only and should not be used for normal operation.
...
cd ~/src/tuya-lights && ./lampctl group list
```
...
If a lamp was repaired, re-paired, or moved to another network, assume the `local_key` may have changed. Read `KEY_EXTRACTION.md` and `ONBOARDING.md`, refresh the key, test with the preferred CLI first (`lampctl <lamp> status`), then update `tuya_lamps.json`.
